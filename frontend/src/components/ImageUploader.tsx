    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative border border-texflow-800 rounded-2xl shadow-2xl w-full max-w-md mx-4" style={{ background: '#0a0c3d' }}>
        <div className="flex items-center justify-between p-4 border-b border-texflow-800">
          <div className="flex items-center gap-2"><ImageIcon size={18} className="text-texflow-400" /><h2 className="text-lg font-semibold text-white">Insert Image</h2></div>
        ...
        <div className="p-4 space-y-4">
          <div onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()} className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${dragging ? 'border-texflow-400 bg-texflow-900/20' : 'border-texflow-800 hover:border-texflow-600'}`}>
            ...
            <p className="text-sm text-slate-300">{fileName || 'Drop an image here or click to browse'}</p>
            <p className="text-xs text-slate-500 mt-1">PNG, JPG, SVG up to 10MB</p>
          </div>
          ...
          <div><label className="block text-xs text-slate-400 mb-1">Caption</label><input value={caption} onChange={e => setCaption(e.target.value)} className="input-field w-full text-sm" placeholder="Image caption" /></div>
          <div className="flex gap-3">
            <div className="flex-1"><label className="block text-xs text-slate-400 mb-1">Label</label><input value={label} onChange={e => setLabel(e.target.value)} className="input-field w-full text-sm" placeholder="fig:label" /></div>
            <div className="w-24"><label className="block text-xs text-slate-400 mb-1">Width</label><select value={width} onChange={e => setWidth(e.target.value)} className="input-field w-full text-sm"><option value="0.5">50%</option><option value="0.8">80%</option><option value="1.0">100%</option></select></div>
          </div>
