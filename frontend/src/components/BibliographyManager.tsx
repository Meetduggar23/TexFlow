    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative border border-texflow-800 rounded-2xl shadow-2xl w-full max-w-lg mx-4" style={{ background: '#0a0c3d' }}>
        <div className="flex items-center justify-between p-4 border-b border-texflow-800">
          <div className="flex items-center gap-2"><BookOpen size={18} className="text-texflow-400" /><h2 className="text-lg font-semibold text-white">Bibliography</h2></div>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-white hover:bg-dark-700 rounded"><X size={18} /></button>
        </div>
        <div className="p-4 space-y-4">
          <div className="flex gap-2">
            <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={14} /><input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search references..." className="input-field w-full pl-8 text-sm" /></div>
            <button onClick={() => setShowAdd(!showAdd)} className="btn-primary text-sm flex items-center gap-1"><Plus size={14} /> Add</button>
          </div>

          {showAdd && (
            <div className="p-3 rounded-lg border border-texflow-800 space-y-2" style={{ background: 'rgba(3,6,55,0.5)' }}>
          ...
          <div className="max-h-64 overflow-auto space-y-1">
            {filtered.length === 0 ? (
              <p className="text-sm text-slate-500 text-center py-4">No bibliography entries found</p>
            ) : filtered.map(entry => (
              <div key={entry.key} className="flex items-center justify-between p-2 rounded-lg hover:bg-dark-700 transition-colors group">
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-white truncate">{entry.title || entry.key}</p>
                  <p className="text-xs text-slate-400">{entry.author} {entry.year && `(${entry.year})`} · <span className="text-texflow-400">{entry.key}</span></p>
                </div>
