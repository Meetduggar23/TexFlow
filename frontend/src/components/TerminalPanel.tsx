  ...
  return (
    <div className={clsx('border-t border-texflow-800 transition-all duration-200', isOpen ? 'h-48' : 'h-8')}>
      ...
      >
        ...
        <div>{isOpen ? <ChevronDown size={14} className="text-slate-400" /> : <ChevronUp size={14} className="text-slate-400" />}</div>
      </div>
      ...
      {isOpen && (
        <div className="h-[calc(100%-32px)] flex flex-col bg-dark-900">
          <div className="flex items-center gap-1 px-3 py-1 border-b border-texflow-800">
            {(['logs', 'errors', 'warnings'] as const).map(tab => (
              <button
                ...
                style={activeTab === tab ? { background: tab === 'errors' ? 'rgba(220,38,38,0.2)' : tab === 'warnings' ? 'rgba(234,179,8,0.2)' : '#3C0753' } : {}}
              >
                {tab}
              </button>
            ))}
          ...
          <div ref={logsRef} className="flex-1 overflow-auto p-3 font-mono text-xs">
            {activeTab === 'logs' && <pre className="text-slate-300 whitespace-pre-wrap">{logs || 'No logs available. Compile your project to see output.'}</pre>}
            {activeTab === 'errors' && (
              <div className="space-y-1">
                {errors.length === 0 ? <p className="text-slate-500">No errors</p> : errors.map((error, i) => (
                  <div key={i} className="flex items-start gap-2 text-red-400">
                    <AlertCircle size={12} className="mt-0.5 flex-shrink-0" />
                    <span>{error.file && <span className="text-slate-400">{error.file}: </span>}Line {error.line}, Col {error.column}: {error.message}</span>
                  </div>
                ))}
              </div>
            )}
            {activeTab === 'warnings' && (
              <div className="space-y-1">
                {warnings.length === 0 ? <p className="text-slate-500">No warnings</p> : warnings.map((warning, i) => (
                  <div key={i} className="flex items-start gap-2 text-yellow-400">
                    <AlertTriangle size={12} className="mt-0.5 flex-shrink-0" />
                    <span>{warning.file && <span className="text-slate-400">{warning.file}: </span>}Line {warning.line}, Col {warning.column}: {warning.message}</span>
                  </div>
                ))}
              </div>
            )}
