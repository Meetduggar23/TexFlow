...
export default function AuthModal({ onClose, onSuccess }: AuthModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative border border-texflow-800 rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden" style={{ background: '#0a0c3d' }}>
        <div className="flex items-center justify-between p-4 border-b border-texflow-800">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold text-white">Tex<span style={{ background: 'linear-gradient(135deg, #910A67, #b97cc9)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Flow</span></span>
          </div>
        ...
        <div className="flex border-b border-texflow-800">
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === 'login' ? 'text-white border-b-2' : 'text-slate-400 hover:text-white'}`}
            style={tab === 'login' ? { borderColor: '#720455' } : {}}
          >
            Log in
          </button>
          <button
            onClick={() => setTab('signup')}
            className={`flex-1 py-3 text-sm font-medium transition-colors ${tab === 'signup' ? 'text-white border-b-2' : 'text-slate-400 hover:text-white'}`}
            style={tab === 'signup' ? { borderColor: '#720455' } : {}}
          >
            Sign up
          </button>
        </div>
