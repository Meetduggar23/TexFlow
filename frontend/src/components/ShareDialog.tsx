...
export default function ShareDialog({ onClose }: ShareDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative border border-texflow-800 rounded-2xl shadow-2xl w-full max-w-lg mx-4" style={{ background: '#0a0c3d' }}>
        <div className="flex items-center justify-between p-4 border-b border-texflow-800">
          <div className="flex items-center gap-2"><Share2 size={18} className="text-texflow-400" /><h2 className="text-lg font-semibold text-white">Share Project</h2></div>
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            {shareLink && (
              <div className="flex items-center gap-2 mt-2 p-2 rounded-lg border border-texflow-800" style={{ background: '#030637' }}>
                <input value={shareLink} readOnly className="flex-1 bg-transparent text-xs text-slate-300 outline-none" />
                <button onClick={handleCopyLink} className="p-1 text-slate-400 hover:text-white">{copied ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}</button>
              </div>
            )}
          </div>
          ...
          {members.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {members.map(member => (
                  <div key={member.id} className="flex items-center justify-between p-2 rounded-lg border border-texflow-800" style={{ background: 'rgba(3,6,55,0.5)' }}>
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs text-white" style={{ background: 'linear-gradient(135deg, #720455, #910A67)' }}>{member.user?.name?.[0] || 'U'}</div>
                      <div><p className="text-sm text-white">{member.user?.name || member.user?.email}</p><p className="text-xs text-slate-500">{member.role}</p></div>
                    </div>
                    <button onClick={() => handleRemoveMember(member.userId)} className="p-1 text-slate-400 hover:text-red-400"><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
            </div>
