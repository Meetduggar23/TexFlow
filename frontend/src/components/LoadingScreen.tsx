...
export default function LoadingScreen({ onComplete }: LoadingScreenProps) {
  ...
  return (
    <div className={`loading-screen transition-opacity duration-500 ${fadeOut ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <img 
          ...
          onError={(e) => {
            ...
            if (parent) {
              ...
              fallback.style.background = 'linear-gradient(135deg, #720455, #910A67)';
              fallback.innerHTML = '<span class="text-white font-bold text-4xl">T</span>';
      ...
      <p className="text-slate-400 text-sm mb-8">Collaborative LaTeX Writing Platform</p>
      
      <div className="loading-bar-container">
        <div className="loading-bar" style={{ width: `${Math.min(progress, 100)}%` }} />
      </div>
      
      <p className="text-xs text-slate-500 mt-4">Loading editor...</p>
    </div>
  );
}
