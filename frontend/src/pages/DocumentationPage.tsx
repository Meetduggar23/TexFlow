...
import { Search, ChevronRight, Copy, Check, ArrowLeft, ArrowRight, Menu } from 'lucide-react';
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={handleCopy} className="flex items-center gap-1 px-2 py-1 text-[11px] rounded transition-colors"
      style={{ color: copied ? 'var(--color-success, #10B981)' : 'var(--color-text-muted)' }}>
...
export default function DocumentationPage() {
  ...
  const searchResults = useMemo(() => {
    ...
    return docSections.filter(s =>
      s.title.toLowerCase().includes(q) || s.category.toLowerCase().includes(q) || s.content.toLowerCase().includes(q)
    );
  }, [searchQuery]);
  ...
  const formatContent = (content: string) => {
    return content.split('\n').map((line, i) => {
      if (line.match(/^\d+\.\s/)) return <li key={i} className="ml-5 list-decimal" style={{ color: 'var(--color-text-secondary)' }}>{line.replace(/^\d+\.\s/, '')}</li>;
      if (line.startsWith('•')) return <li key={i} className="ml-5 list-disc" style={{ color: 'var(--color-text-secondary)' }}>{line.slice(2)}</li>;
      if (line.startsWith('Q:')) return <p key={i} className="font-medium mt-4 mb-1" style={{ color: 'var(--color-text-primary)' }}>{line}</p>;
      if (line.startsWith('A:')) return <p key={i} className="ml-4 mb-1" style={{ color: 'var(--color-text-secondary)' }}>{line}</p>;
      if (line.trim() === '') return <div key={i} className="h-2" />;
      return <p key={i} className="mb-2 leading-relaxed" style={{ color: 'var(--color-text-secondary)', fontSize: '14px' }}>{line}</p>;
    });
  };
  ...
  const SidebarContent = () => (
    <nav className="p-3 space-y-4">
      {categories.map(cat => (
        <div key={cat}>
          <h3 className="text-[10px] font-bold uppercase tracking-widest mb-1.5 px-2" style={{ color: 'var(--color-text-muted)' }}>{cat}</h3>
          <div className="space-y-0.5">
            {docSections.filter(s => s.category === cat).map(s => (
              <button key={s.id} onClick={() => { setActiveId(s.id); setSearchQuery(''); setMobileMenuOpen(false); }}
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--color-background)' }}>
      {/* Top bar with search */}
      <div className="flex-shrink-0 px-4 md:px-8 py-4 border-b" style={{ borderColor: 'var(--color-border)' }}>
        <div className="flex items-center gap-3 max-w-5xl mx-auto">
          <button onClick={() => setMobileMenuOpen(p => !p)} className="md:hidden p-1.5 rounded-lg" style={{ color: 'var(--color-text-muted)' }}>
            <Menu size={18} />
          </button>
          <h1 className="text-sm font-semibold hidden md:block" style={{ color: 'var(--color-text-primary)' }}>Documentation</h1>
          <div className="relative flex-1 max-w-md">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2" size={14} style={{ color: 'var(--color-text-muted)' }} />
            <input
              type="text" placeholder="Search documentation..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-4 py-2 text-[13px] rounded-lg border outline-none"
              style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)', color: 'var(--color-text-primary)' }}
            />
            {searchQuery && (
              <div className="absolute top-full left-0 right-0 mt-1 rounded-lg border shadow-xl z-50 max-h-64 overflow-auto"
                style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border-strong)' }}>
                {searchResults.length === 0 ? (
                  <div className="p-4 text-center text-[13px]" style={{ color: 'var(--color-text-muted)' }}>No results found</div>
                ) : searchResults.map(s => (
                  <button key={s.id} onClick={() => { setActiveId(s.id); setSearchQuery(''); }}
                    className="w-full text-left px-3 py-2 text-[13px] border-b last:border-0 transition-colors"
                    style={{ borderColor: 'var(--color-border)' }}
                    onMouseEnter={e => e.currentTarget.style.background = 'var(--color-surface-elevated)'}
                    onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                  >
                    <span className="text-[10px] font-semibold uppercase" style={{ color: 'var(--color-accent)' }}>{s.category}</span>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-40" style={{ top: 0 }}>
          ...
          <div className="absolute left-0 top-0 bottom-0 w-64 overflow-y-auto" style={{ background: 'var(--color-surface)' }}>
            <div className="p-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
              <h2 className="text-xs font-bold uppercase tracking-wider" style={{ color: 'var(--color-text-muted)' }}>Documentation</h2>
            </div>
            <SidebarContent />
          </div>
        </div>
      ...
      <div className="flex-1 flex min-h-0">
        {/* Desktop sidebar */}
        <aside className="hidden md:flex w-56 lg:w-60 flex-shrink-0 flex-col border-r overflow-y-auto"
          style={{ borderColor: 'var(--color-border)', background: 'var(--color-surface)' }}>
          <div className="p-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
            <h2 className="text-[11px] font-bold uppercase tracking-wider" style={{ color: 'var(--color-text-muted)' }}>Documentation</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-3xl mx-auto px-4 md:px-8 py-6 md:py-8">
            ...
            <div className="flex items-center gap-1.5 text-[12px] mb-4" style={{ color: 'var(--color-text-muted)' }}>
              ...
              <span style={{ color: 'var(--color-text-secondary)' }}>{activeSection.category}</span>
              <ChevronRight size={12} />
              <span style={{ color: 'var(--color-text-primary)' }}>{activeSection.title}</span>
            </div>

            {/* Title */}
            <h1 className="text-2xl font-bold mb-2" style={{ color: 'var(--color-text-primary)' }}>{activeSection.title}</h1>
            <p className="text-[13px] mb-6 pb-6 border-b" style={{ color: 'var(--color-text-muted)', borderColor: 'var(--color-border)' }}>
            ...
            {/* Code block */}
            {activeSection.code && (
              <div className="mt-6 rounded-lg border overflow-hidden" style={{ borderColor: 'var(--color-border)' }}>
                <div className="flex items-center justify-between px-4 py-2 border-b" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            ...
            {activeSection.related && activeSection.related.length > 0 && (
              <div className="mt-8 pt-6 border-t" style={{ borderColor: 'var(--color-border)' }}>
                <h3 className="text-[11px] font-bold uppercase tracking-wider mb-3" style={{ color: 'var(--color-text-muted)' }}>Related</h3>
            ...
            <div className="mt-10 pt-6 border-t flex justify-between" style={{ borderColor: 'var(--color-border)' }}>
              {prevSection ? (
                <button onClick={() => setActiveId(prevSection.id)} className="flex items-center gap-2 text-[13px] transition-colors group"
                  style={{ color: 'var(--color-text-muted)' }}
                  onMouseEnter={e => e.currentTarget.style.color = 'var(--color-accent)'}
                  onMouseLeave={e => e.currentTarget.style.color = 'var(--color-text-muted)'}
                ><ArrowLeft size={14} className="group-hover:-translate-x-0.5 transition-transform" /> {prevSection.title}</button>
              ) : <div />}
              {nextSection ? (
                <button onClick={() => setActiveId(nextSection.id)} className="flex items-center gap-2 text-[13px] transition-colors group"
                  style={{ color: 'var(--color-text-muted)' }}
                  onMouseEnter={e => e.currentTarget.style.color = 'var(--color-accent)'}
                  onMouseLeave={e => e.currentTarget.style.color = 'var(--color-text-muted)'}
                >{nextSection.title} <ArrowRight size={14} className="group-hover:translate-x-0.5 transition-transform" /></button>
              ) : <div />}
            </div>
          </div>
        </div>
