    ...
    <div className="p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-slate-400 mt-1">Start from a professionally designed template</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {categories.map(cat => (
          <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3 py-1.5 text-sm rounded-full transition-all ${selectedCategory === cat ? 'text-white' : 'text-slate-400 border border-texflow-700 hover:border-texflow-500'}`} style={selectedCategory === cat ? { background: 'linear-gradient(135deg, #720455, #910A67)' } : {}}>
            {cat}
          </button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(template => (
            <div key={template.id} className="card group">
              <div className="w-full h-32 rounded-lg mb-4 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, rgba(114,4,85,0.15), rgba(145,10,103,0.1))' }}>
                <FileText size={40} className="text-texflow-400 opacity-50" />
              </div>
              <div className="flex items-start justify-between mb-2">
                <div>
                  <h3 className="font-semibold text-white group-hover:text-texflow-300 transition-colors">{template.name}</h3>
